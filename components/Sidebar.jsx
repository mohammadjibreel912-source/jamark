...
{step >= 4 &&
          (companyName || activityId || companyType || companyForm || managementMethod || managerName || factoryName || factoryLocation) && (
            <div style={{ display: 'flex', alignItems: 'stretch', gap: '10px', marginBottom: '10px' }}>
              <VerticalLine />
              {/* Text content */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                ...
                <div>
                  ...
                  {companyName && <div>{t.companyName}: {companyName}</div>}
                  {activityId && <div>{t.activity}: {activityId}</div>}
                  {managementMethod && <div>{t.managementMethod}: {managementMethod}</div>}
                  {managerName && <div>{t.managerName}: {managerName}</div>}

                  {/* 3. حقول الشركة (إذا لم يكن مصنعاً) */}
                  {!isFactory && companyType && <div>{t.companyType}: {companyType}</div>}
                  {!isFactory && companyForm && <div>{t.companyForm}: {companyForm}</div>}
                  ...
                  {isFactory && (
                    <>
                      {factoryName && <div>{t.factoryName}: {factoryName}</div>}
                      {factoryLocation && <div>{t.factoryLocation}: {factoryLocation}</div>}
                      {foundationYear && <div>{t.foundationYear}: {foundationYear}</div>}
          ...
          <div style={{ display: 'flex', alignItems: 'stretch', gap: '10px', marginBottom: '10px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              ...
              <div style={{ fontSize: '14px' }}>
                ...
                {/* الموقع الجغرافي */}
                {establishmentLocation && <div style={{ overflowWrap: 'break-word' }}>{establishmentLocation}</div>}
